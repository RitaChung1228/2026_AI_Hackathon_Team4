...
import type { AppNotification, AuthUser, CartItem, ScheduledTrip } from "../types";
...
interface HomeProps {
  ...
  onNotificationSelect?: (n: AppNotification) => void;
}
...
export default function Home({
  ...
  const renderCard = (r: RecoItem, because?: string) => {
    ...
    return (
      ...
      >
        {/* 圖片 / 服務磁磚 */}
        <div style={{ position: "relative", height: 96, flexShrink: 0, background: r.tint ?? "#F1F5F9" }}>
          {r.image ? (
            ...
            <div
              style={{
                ...
                fontSize: 34,
                background: `linear-gradient(135deg, ${r.tint ?? "#E7EEF5"}, white)`,
              }}
            >
              {r.icon}
            </div>
          ...
          <span
            style={{
              ...
              fontSize: 10,
              fontWeight: 700,
              color: r.kind === "service" ? "#4C6E91" : "#16A34A",
        ...
        <div style={{ padding: "10px 11px 11px", display: "flex", flexDirection: "column", flex: 1 }}>
          <div style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 13, color: "#16232E", lineHeight: 1.3 }}>
            {r.name}
          </div>
          <div style={{ fontSize: 11, color: "#94A3B8", marginTop: 3, lineHeight: 1.4 }}>{r.detail}</div>
          <div style={{ fontSize: 10, color: "#64748B", marginTop: 4 }}>🏬 {r.vendor}</div>

          {because && (
            <div style={{ fontSize: 10, color: "#4C6E91", marginTop: 6, lineHeight: 1.4, fontWeight: 600 }}>
              ✦ 常與「{because}」一起準備
            </div>
          )}
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 6 }}>
            <span style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: 15, color: "#16232E" }}>
            ...
            <button
              ...
              style={{
                ...
                background: inCart ? "white" : "linear-gradient(135deg, #4C6E91, #6E92B4)",
    ...
    <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 12, gap: 10 }}>
      ...
      {action && (
        <button
          ...
          style={{ fontSize: 12, color: "#4C6E91", background: "none", border: "none", cursor: "pointer", fontWeight: 600, fontFamily: "var(--font-display)", whiteSpace: "nowrap", flexShrink: 0, padding: 0 }}
        >
          {action.label}
        </button>
      )}
  ...
  return (
    <div style={{ height: "100%", overflowY: "auto", background: "#F5F7FA", paddingBottom: 90 }} className="scrollbar-hide">
      {/* Header */}
      <div style={{ background: "white", padding: "56px 20px 18px", borderBottom: "1px solid #F1F5F9" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <img src="/logo-mark.png" alt="Lifepack" style={{ width: 32, height: 32, objectFit: "contain" }} />
            <span style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: 17, color: "#16232E", letterSpacing: "-0.3px" }}>
              Lifepack
            </span>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <NotificationBell
              ...
              onOpen={onNotificationsRead}
              onSelect={onNotificationSelect}
            />
            {cartItems.length > 0 && (
              <button
                onClick={onOpenCart}
                style={{ width: 36, height: 36, borderRadius: "50%", background: "#F1F5F9", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, position: "relative" }}
              >
                🛒
            ...
            <AvatarView value={displayAvatar} size={36} fontSize={13} />
          </div>
        </div>

        <h2 style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: 24, color: "#16232E", margin: 0, marginBottom: 6, letterSpacing: "-0.5px" }}>
          Hi {displayName}
        </h2>
